...
public final class NexusGateCommandExecutor implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "setpassword": {
                if (args.length < 2) {
                    sender.sendMessage(GateListener.colorize("&cUsage: /nexusgate setpassword <password>"));
                    return true;
                }
                String newPassword = String.join(" ", Arrays.copyOfRange(args, 1, args.length));
                passwordManager.setPassword(newPassword);
                sender.sendMessage(GateListener.colorize("&aNexusGate password updated."));
                ...
                plugin.getLogger().info("[NexusGate] Password changed by " + sender.getName() + ".");
                return true;
            }
            case "reload": {
            ...
            case "status": {
                ...
                sender.sendMessage(GateListener.colorize("&7Password set: " + (passwordManager.isSet() ? "&aYES" : "&cNO")));
    ...
    private void sendUsage(CommandSender sender) {
        sender.sendMessage(GateListener.colorize("&6NexusGate &7-- /nexusgate <setpassword|reload|status>"));
    }
}
