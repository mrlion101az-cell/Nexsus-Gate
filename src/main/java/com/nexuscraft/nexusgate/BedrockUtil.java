...
/**
 * Best-effort Bedrock detection so Bedrock players (already handled through
 * your other setup) can skip the password gate entirely.
 *
 * Deliberately uses reflection instead of a compile-time Floodgate
 * dependency: Floodgate is only a soft-dependency (plugin.yml softdepend),
 * so this class must still compile and run fine on a server that doesn't
 * have Floodgate installed at all.
 *
 * If Floodgate isn't present/reachable, falls back to the well-known
 * community heuristic that Floodgate assigns UUID version 0 to Bedrock
 * players who don't have a linked Java account. That fallback is NOT as
 * reliable as asking Floodgate directly (a real Java player could in
 * principle have a version-0 UUID, though it's exceedingly rare since
 * Mojang UUIDs are version 4 or version 3), so it's config-toggleable.
 */
public final class BedrockUtil {
    ...
    public static boolean isBedrockPlayer(Player player, Logger logger) {
        ...
        return isLikelyBedrockUuid(player.getUniqueId());
    }
