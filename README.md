...
## New in v0.5.0: ban-evasion detection now defaults to auto-ban

Requested explicitly, after v0.4.0 shipped alert-only: turn ban-evasion detection into an actual
auto-ban, no human required. As shipped, **`config.yml` now has `alt-detection.mode:
"autoban"`**: the instant a brand-new account connects from an IP that's also in a
currently-banned player's IP history, that connection is rejected on the spot and the new
account is auto-banned (reason: "Auto-ban: shared connection with banned player(s) ..."). With
`alt-detection.ban-ip-too: true` (also the default), the IP itself gets auto-banned too, so a
repeat attempt from a fresh account on the same connection is turned away at the door as well.

**Read this before you forget it's on:** this is a heuristic, not a certainty. It cannot tell
the difference between "the same person evading a ban" and "two roommates, a family, a dorm
floor, or a phone on mobile data that happens to share an IP with someone who got banned." Both
look identical from the server's side. Every auto-ban still fires a live alert to everyone
holding `nexusgate.watch` and writes an `AUTOBAN` line to `auth-log.txt`, specifically so a false
positive gets caught fast -- undo one with `/nexusgate unban <player>` and, if it was also
IP-banned, `/nexusgate unbanip <ip>`. If this ever locks out someone innocent and you'd rather
go back to alert-only (never blocks, just pings staff and logs an `ALT-ALERT`), change
`alt-detection.mode` to `"watch"` in `config.yml` and run `/nexusgate reload` -- takes effect
immediately, no restart. `/nexusgate status` shows the current mode at a glance.

## From v0.4.0: IP bans, and the ban-evasion detection this builds on

- **`/nexusgate banip <ip> [reason]`** -- a deliberate, admin-decided ban on a whole connection,
  not just one account. Checked at the same earliest possible point as a normal ban and DOES
  reject the connection, because it's a human decision, not a guess. `/nexusgate unbanip <ip>`
  reverses it, `/nexusgate ipbanlist` shows what's banned.
- **Ban-evasion detection** watches for a brand-new account connecting from an IP that's also on
  file for a currently-banned player. What happens next depends on `alt-detection.mode` above.
  `/nexusgate altcheck <player>` always works regardless of mode: it shows that player's IP
  history and everyone else who's shared each of those IPs (flagging the banned ones), for
  reviewing any alert or auto-ban after the fact. The same `nexusgate.watch` audience also gets
  pinged the first time any gamer tag ever successfully logs in.
...
class this plugin touches (`stubs/`, shipped in this zip) and compiled the real source against
those by hand with `javac` -- clean, zero errors, zero warnings (`-Xlint:all`) for the v0.1.0
code and everything added since, including this v0.5.0 change. That confirms the Java itself is
internally consistent, but not that every signature matches the real jar exactly for your exact
server build.
...
- `/nexusgate banip <ip> [reason]` -- bans a whole connection outright, independent of any one
  account. Kicks anyone currently online from that IP. (`nexusgate.admin`)
- `/nexusgate unbanip <ip>` -- reverses it. (`nexusgate.admin`)
- `/nexusgate ipbanlist` -- every currently IP-banned address, why, by whom, and when.
- `/nexusgate altcheck <player>` -- that player's recent IP history, and everyone else who's
  ever shared each of those IPs (flagging anyone banned) -- the tool for deciding, as a human,
  whether an `ALT-ALERT` was actually the same person evading a ban or just a shared network.
