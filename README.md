...
one. That's now in: Bedrock/Xbox connections go through the exact same freeze-until-`/login`
flow Java players always have, but checked against their own password, set independently with
`/nexusgate setpassword bedrock <password>` (Java's is now set with
`/nexusgate setpassword java <password>` -- the old two-word form, `/nexusgate setpassword
<password>`, still works exactly as before and still means "set the Java password", so nothing
you already had memorized breaks).
...
- `/nexusgate setpassword [java|bedrock] <password>` -- set/change one platform's password.
  Omitting `java`/`bedrock` sets the Java password (old v0.1.0 form, still works).
  (`nexusgate.admin`, default: op)
